...
async function signInWith(provider: 'github' | 'google') {
  ...
  const redirectTo = `${window.location.origin}/auth/callback?redirect=${encodeURIComponent((route.query.redirect as string) || '/dashboard')}`
  const { error: e } = await supabase.auth.signInWithOAuth({ provider, options: { redirectTo } })
  if (e) { error.value = e.message; loading.value = null }
}
</script>
...
<template>
  <div class="min-h-[60vh] flex items-center justify-center px-4">
    <div class="w-full max-w-sm">
      ...
      <div class="space-y-3">
        <button type="button" :disabled="!!loading" class="w-full flex items-center justify-center gap-3 py-3 rounded-lg border border-vex-border bg-vex-surface hover:bg-vex-surface-light text-white font-medium transition-colors cursor-pointer disabled:opacity-50" @click="signInWith('github')">
        ...
        <button type="button" :disabled="!!loading" class="w-full flex items-center justify-center gap-3 py-3 rounded-lg border border-vex-border bg-vex-surface hover:bg-vex-surface-light text-white font-medium transition-colors cursor-pointer disabled:opacity-50" @click="signInWith('google')">
      ...
      <div class="mt-8 pt-6 border-t border-vex-border">
        <p class="text-xs text-vex-text-muted text-center leading-relaxed">By signing in, you agree to our Terms of Service.<br>Authentication is handled securely by GitHub and Google.</p>
      </div>
    </div>
  </div>
</template>
